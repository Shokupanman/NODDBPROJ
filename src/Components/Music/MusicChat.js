...
import "./MusicChat.css";
import axios from "axios";
import { withRouter } from "react-router-dom";

class MusicChat extends Component {
  constructor() {
    super();
    this.state = {
      userId: 0,
  ...
  deletePost(userId) {
    axios.delete(`/api/MusicChat/${userId}`).then(res => {
      this.setState({
        messages: res.data
      });
    });
  ...
  editPost(userId, body) {
    ...
    axios.put(`/api/MusicChat/${userId}`, { body }).then(res => {
...
export default withRouter(MusicChat);
